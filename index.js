...
app.get("/keycaps/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const keycap = await pool.query("SELECT * FROM keycap WHERE keycap_id = $1", [id]);
...
app.post("/keycaps", async (req, res) => {
    try {
        ...
        const newKeycap = await pool.query("INSERT INTO keycap (name, price, description) VALUES ($1, $2, $3) RETURNING *",
...
app.delete("/keycaps/:id", async (req, res) => {
    try { 
        ...
        const deleteKeycap = await pool.query("DELETE FROM keycap WHERE keycap_id = $1",
        [id])
